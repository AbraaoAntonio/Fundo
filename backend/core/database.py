...
class DatabaseManager:
        ...
        async with self._init_lock:
            ...
            try:
                ...
                if using_pgbouncer:
                    ...
                    logger.info("Detected PgBouncer/Supabase pooler: statement_cache_size=0")
                ...
                if is_lambda or using_pgbouncer:
                    # Lambda OR PgBouncer: avoid client-side pooling / connection state conflicts
                    engine_kwargs["poolclass"] = NullPool
                    logger.info("Using NullPool (Lambda or PgBouncer pooler)")
                else:
                    # Direct DB: use connection pooling
                    engine_kwargs["pool_size"] = 10
                    engine_kwargs["max_overflow"] = 20
                    engine_kwargs["pool_recycle"] = 3600
                    engine_kwargs["pool_timeout"] = 30
                    logger.info("Using QueuePool settings for non-Lambda direct DB connection")
    ...
    async def check_and_repair_existing_tables(self):
        ...
        try:
            ...
            async def repair_with_semaphore(table_name):
                ...
                logger.info(f"Table {table_name} repaired in {time.time() - start:.2f}s")
        ...
        try:
            ...
            for column in table.columns:
                ...
                if column.default is not None:
                    default_value = str(getattr(column.default, "arg", column.default))
                elif column.server_default is not None:
                    default_value = str(getattr(column.server_default, "arg", column.server_default))
    ...
    def _find_missing_columns(self, existing_columns, model_columns):
        ...
        return [model_col for model_col in model_columns if model_col["name"] not in existing_names]
    ...
    def _generate_add_column_sql(self, table_name: str, column_info: dict):
        ...
        if default is not None:
            if default == "":
                ...
                if column_type.upper() in ["TEXT", "VARCHAR", "STRING"] and not str(default).isdigit():
